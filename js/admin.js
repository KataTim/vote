...
(function () {
  ...
  function ensurePublishSettingsUI() {
    if (document.getElementById("poll-publish-settings")) return;

    const form = document.getElementById("poll-form");
    const submit = document.getElementById("poll-form-submit");
    if (!form || !submit) return;

    const wrap = document.createElement("div");
    wrap.id = "poll-publish-settings";
    wrap.className = "field";
    wrap.style.cssText = "margin:12px 0;padding:12px;border:1px solid currentColor;opacity:.95;";
    wrap.innerHTML = `
      <div style="font-weight:700;margin-bottom:8px;">投票访问设置</div>
      <label style="display:block;cursor:pointer;margin:7px 0;">
        <input type="checkbox" id="f-allow-unverified" style="pointer-events:auto;cursor:pointer;">
        允许未验证用户参与
      </label>
      <label id="f-auto-publish-wrap" style="display:block;cursor:pointer;margin:7px 0;">
        <input type="checkbox" id="f-auto-publish" style="pointer-events:auto;cursor:pointer;">
        创建后立即发布
      </label>
      <div style="font-size:12px;opacity:.7;line-height:1.5;">
        默认只有已通过身份验证的用户可以投票。
      </div>`;
