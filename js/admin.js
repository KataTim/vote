...
(function () {
  ...
  function ensurePollAccessControls() {
    if (document.getElementById("f-allow-unverified")) return;
    ...
    box.id = "poll-access-settings";
    box.className = "field";
    box.innerHTML = `
      <div style="margin-top:10px;padding:10px;border:1px solid #ccc;background:rgba(128,128,128,.06);">
        <label style="display:block;margin-bottom:8px;">
          <input type="checkbox" id="f-allow-unverified">
          允许未验证用户参与投票
        </label>
        <label id="f-auto-publish-label" style="display:block;">
          <input type="checkbox" id="f-auto-publish">
          创建后立即发布
        </label>
        <div style="font-size:12px;opacity:.75;margin-top:6px;">
          默认只有已通过身份验证的登录用户可以参与；勾选后，已登录但尚未验证的用户也可以参与。
        </div>
      </div>
    `;
  ...
  function getAllowUnverifiedValue() {
    const el = document.getElementById("f-allow-unverified");
    return !!(el && el.checked);
  }

  function getAutoPublishValue() {
    const el = document.getElementById("f-auto-publish");
    return !!(el && el.checked);
  }

  function setCreateOnlyPublishControl(isCreateMode) {
    const label = document.getElementById("f-auto-publish-label");
    const el = document.getElementById("f-auto-publish");
    if (!label || !el) return;

    label.style.display = isCreateMode ? "" : "none";
    if (!isCreateMode) el.checked = false;
  }
  ...
  function resetFormToCreateMode() {
    ...
    ensurePollAccessControls();
    setCreateOnlyPublishControl(true);
    const allowUnverifiedEl = document.getElementById("f-allow-unverified");
    if (allowUnverifiedEl) allowUnverifiedEl.checked = false;
    const autoPublishEl = document.getElementById("f-auto-publish");
    if (autoPublishEl) autoPublishEl.checked = false;
    clearOptionRows();
    addOptionRow("");
    addOptionRow("");
  ...
  function loadPollIntoForm(poll) {
    ...
    ensurePollAccessControls();
    setCreateOnlyPublishControl(false);
    ...
    const allowUnverifiedEl = document.getElementById("f-allow-unverified");
    if (allowUnverifiedEl) {
      allowUnverifiedEl.checked = !!(settings && settings.allow_unverified_users);
    }
  ...
  async function handleFormSubmit(e) {
    ...
    const allowUnverified = getAllowUnverifiedValue();
    const autoPublish = getAutoPublishValue();
  ...
  async function createPoll(p) {
    ...
    setFormMsg(
      null,
      "投票创建成功！编号：" + poll.public_id +
      (poll.is_published ? "（已立即发布）" : "（当前为未发布状态，请在下方列表点击「发布」）")
    );
  ...
  async function refreshList() {
    ...
    data.forEach((poll) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          ${Utils.escapeHtml(poll.title)}
          ${(() => {
            const s = Array.isArray(poll.poll_settings) ? poll.poll_settings[0] : poll.poll_settings;
            return s && s.allow_unverified_users
              ? '<div style="font-size:11px;margin-top:3px;opacity:.75;">允许未验证用户</div>'
              : '<div style="font-size:11px;margin-top:3px;opacity:.55;">仅已验证用户</div>';
          })()}
        </td>
