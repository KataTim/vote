...
(function () {
  ...
  async function handleFormSubmit(e) {
    ...
    try {
      if (!editingPollId) {
        await createPoll({ title, description, startISO, endISO, allowMulti, showResults, randomize, options, userId: user.id });
      } else {
        await updatePoll({ pollId: editingPollId, title, description, startISO, endISO, allowMulti, showResults, randomize, options });
      }
    } finally {
  ...
  async function createPoll(p) {
    ...
    setFormMsg(null, "投票创建成功！编号：" + poll.public_id + "（当前为未发布状态，请在下方列表点击「发布」）");
